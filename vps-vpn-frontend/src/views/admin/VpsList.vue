...
function hasilPing(v) {
  ...
  const hasil = p.status === "down" ? "Tidak ada balasan" : "Ada balasan";
  return `${hasil}, loss ${p.packet_loss}%` +
    (p.rtt_avg_ms != null ? `, RTT ${p.rtt_avg_ms} ms` : "");
}

function waktuPing(v) {
  const waktu = v.ping_terakhir?.checked_at;
  return waktu ? tanggalWaktu(waktu) : "Belum diperiksa";
}
...
<template>
  <section class="card">
    ...
    <p class="section-subtitle">
      Alamat IP hanya terlihat di sini. Halaman publik hanya menampilkan nama dan keterangan.
    </p>
    ...
    <p class="section-subtitle">
      Periksa semua VPS menjalankan ping baru ke seluruh VPS dan memperbarui hasilnya satu per satu.
      Pemeriksaan otomatis setiap 5 menit; status menjadi DOWN setelah 3 pemeriksaan gagal berturut-turut.
    </p>
      ...
      <table class="table">
        <thead>
          <tr>
            ...
            <th>Keterangan</th>
            <th style="min-width: 250px">Status &amp; ping terakhir</th>
        ...
        <tbody>
          ...
          <tr v-for="v in daftar" :key="v.id">
            ...
            <td>
              ...
              <div style="margin-top: 6px">{{ hasilPing(v) }}</div>
              <div v-if="v.ping_terakhir?.status === 'down'" style="margin-top: 4px">
                Gagal {{ v.gagal_berturut }} kali berturut-turut (ambang: 3).
                <span v-if="v.status_terakhir !== 'down'">Menunggu konfirmasi DOWN.</span>
              </div>
