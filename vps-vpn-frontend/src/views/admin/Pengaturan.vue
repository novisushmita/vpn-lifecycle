...
<template>
  <section class="card">
    ...
    <p class="section-subtitle">
      Parameter operasional layanan VPN. Konfigurasi dasar router seperti alamat
      interface, rute, dan aturan firewall fondasi sengaja tidak diatur dari
      sini karena kesalahan pada bagian itu dapat memutus jalur sistem ke router.
    </p>
      ...
      <div v-else class="form-grid">
        <div v-for="p in daftar" :key="p.kunci" class="field">
          <label class="field-label">
            {{ p.label }}
            <span v-if="p.router" class="badge badge-warning" style="margin-left: 6px">
              diterapkan ke router
            </span>
          </label>
