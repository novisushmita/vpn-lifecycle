...
function ddmmyy(nilai) {
  return Object.fromEntries(
    new Intl.DateTimeFormat("en-GB", {
      timeZone: ZONA, day: "2-digit", month: "2-digit", year: "2-digit",
    })
      .formatToParts(nilai instanceof Date ? nilai : new Date(nilai))
      .filter((x) => x.type !== "literal")
      .map((x) => [x.type, x.value])
  );
}

/** 06/09/26 — tanggal saja, bentuk padat */
export function tanggalSingkat(nilai) {
  const b = bagian(nilai);
  if (!b) return "-";
  const p = ddmmyy(nilai);
  return `${p.day}/${p.month}/${p.year}`;
}

/** 06/09/26 14:31:05 — bentuk padat untuk tabel/grafik yang rapat */
export function waktuSingkat(nilai) {
  const b = bagian(nilai);
  if (!b) return "-";
  const p = ddmmyy(nilai);
  return `${p.day}/${p.month}/${p.year} ${b.jam}:${b.menit}:${b.detik}`;
}
