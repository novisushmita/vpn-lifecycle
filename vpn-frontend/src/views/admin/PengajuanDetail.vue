...
import { tanggalSingkat, waktuSingkat } from "../../lib/format";
...
<template>
  ...
  <template v-if="p">
    <div class="page-head">
      <button class="btn-kembali" @click="router.push('/admin/pengajuan')">
      ...
      <div class="page-head__meta">
        Diajukan {{ waktuSingkat(p.dibuat_pada) }}
        <template v-if="p.ditinjau_pada">
          · ditinjau {{ waktuSingkat(p.ditinjau_pada) }}
        ...
        <section class="card card--tight">
          ...
          <dl class="kv">
            ...
            <div class="kv__row">
              <dt>Masa akses</dt>
              <dd>
                {{ tanggalSingkat(p.durasi_mulai) }} sampai {{ tanggalSingkat(p.durasi_selesai) }}
        ...
        <div v-else-if="p.status === 'disetujui'" class="panel">
          ...
          <div class="panel__body" style="font-size: 12.5px; color: var(--color-text-muted)">
            <p style="margin: 0 0 8px">
              Disetujui {{ waktuSingkat(p.ditinjau_pada) }}<template v-if="p.ditinjau_oleh"> oleh
        ...
        <div v-else-if="p.status === 'ditolak'" class="panel">
          ...
          <div class="panel__body">
            <p style="font-size: 12.5px; color: var(--color-text-muted); margin: 0 0 8px">
              Ditolak {{ waktuSingkat(p.ditinjau_pada) }}<template v-if="p.ditinjau_oleh"> oleh
              {{ p.ditinjau_oleh }}</template>.
            </p>
